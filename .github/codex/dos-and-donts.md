...
2. **Require standard tags** on every resource, at least `Project` and `BillingCode`.
...
2. **Don't hardcode secrets** in `.tf` files, tfvars committed to git, or CI logs. Use a secrets manager / OIDC / CI secret store.
3. **Don't apply from a laptop to shared/prod accounts.** Changes go through the pipeline (plan → review → apply) with the designated runner role.
